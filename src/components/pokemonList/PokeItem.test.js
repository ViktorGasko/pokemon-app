...
test("renders pokemon image", () => {
  ...
  expect(sprite).toHaveAttribute('src', 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/1.png');
});
test("renders pokemon backup image", () => {
  render(<PokeItem pokemon={{name: 'bulbasaur', url: ""}}/>);
  const sprite = screen.getByRole('img');
